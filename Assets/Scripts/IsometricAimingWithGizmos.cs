    ...
    public class IsometricAimingWithGizmos : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (hitPosition != null)
            {
                ...
                if (lookAtPoint == null)
                {
                    lookAtPoint = GameObject.Find("PlayerLookAtPoint");
                }

                // Update the position of lookAtPoint based on hitPosition
                lookAtPoint.transform.position = hitPosition;
            }
            else
            {
                // If hitPosition is null, destroy the lookAtPoint GameObject
                if (lookAtPoint != null)
                {
                    Destroy(lookAtPoint);
                }
        ...
        void SetHitPosition(Vector3 position)
        {
            hitPosition = position;
        }
